...
/**
 * A single vector object (a path-painting operation) extracted from a page's content stream.
 * Foundation for Stage 3 (d) vector-object select/delete.
 *
 * <p>Geometry is in PDF user space (points, bottom-left origin), mirroring {@link
 * PdfJsonImageElement} so the frontend can reuse the same bounds-to-CSS helpers. {@code deleted} is
 * set by the editor; the regenerate path omits deleted paths.
 */
@Data
@Builder
@NoArgsConstructor
